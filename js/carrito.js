...
function renderListaCarrito() {
  ...
  document.getElementById('carrito-total').textContent    = `BOB ${precioTotal}`;
}
...
async function abrirModalReserva() {
  ...
  document.getElementById('reserva-subtitulo').textContent =
    `${cantidad} servicio${cantidad > 1 ? 's' : ''} — ${duracionTotal} min — BOB ${precioTotal}`;
...
function construirMensajeWhatsapp(datos) {
  const listaServicios = datos._serviciosData.map(s => `   • ${s.nombre} — BOB ${s.precio}`).join('\n');
